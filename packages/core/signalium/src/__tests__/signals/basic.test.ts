...
import { state, createComputed } from '../utils/instrumented-signals.js';
...
describe('Basic Signal functionality', () => {
  ...
  describe('Propagation', () => {
    test('it works with multiple parents', () => {
      ...
      const outer1 = createComputed(() => {
        return inner.get() + c.get();
      });

      const outer2 = createComputed(() => {
