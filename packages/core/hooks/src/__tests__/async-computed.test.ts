...
import { createAsyncComputed } from '../context.js';
import { state } from 'signalium';
import { nextTick } from './utils/async.js';

describe('async computeds', () => {
  test('Basic async computed works', async () => {
    const getC = createAsyncComputed(async (a: number, b: number) => {
      return a + b;
    });
  ...
  test('Async computed is recomputed when state changes', async () => {
    ...
    const getC = createAsyncComputed(async (a: number) => {
      computeCount++;
  ...
  test('Async computed handles errors', async () => {
    const getC = createAsyncComputed(async (shouldError: boolean) => {
      if (shouldError) {
  ...
  test('Async computed with init value starts ready', () => {
    const getC = createAsyncComputed(async () => 'updated', { initValue: 'initial' });
  ...
  test('Nested async computeds work correctly', async () => {
    ...
    const inner = createAsyncComputed(async (x: number) => {
      innerCount++;
      await nextTick();
      return x * 2;
    });

    const outer = createAsyncComputed(async (x: number) => {
      outerCount++;
  ...
  test('Nested async computeds handle errors correctly', async () => {
    const inner = createAsyncComputed(async (shouldError: boolean) => {
      if (shouldError) throw new Error('Inner error');
      await nextTick();
      return 'inner success';
    });

    const outer = createAsyncComputed(async (shouldError: boolean) => {
  ...
  test('Nested async computeds with init values work correctly', async () => {
    const inner = createAsyncComputed(async (x: number) => x * 2, { initValue: 0 });
    const outer = createAsyncComputed(
      async (x: number) => {
        const innerResult = inner(x);
        await innerResult.await();
