import { MutationObserver, MutationOptions, QueryClient, notifyManager } from '@tanstack/query-core';
import { Atom, Getter, atom } from 'jotai';
import { queryClientAtom } from './queryClientAtom.js';
import { AtomWithMutationResult, MutateFunction } from './types.js';
import { shouldThrowError } from './utils.js';

export function atomWithMutation<TData = unknown, TVariables = void, TError = unknown, TContext = unknown>(
  getOptions: (get: Getter) => MutationOptions<TData, TError, TVariables, TContext>,
  ...
  const observerCacheAtom = atom(
    () => new WeakMap<QueryClient, MutationObserver<TData, TError, TVariables, TContext>>(),
  );
  if (process.env.NODE_ENV !== 'production') {
  ...
  const mutateAtom = atom(get => {
    const observer = get(observerAtom);
    const mutate: MutateFunction<TData, TError, TVariables, TContext> = (variables, options) => {
  ...
  return atom(get => {
    ...
    if (result.isError && shouldThrowError(observer.options.throwOnError, [result.error])) {
      throw result.error;
    }
