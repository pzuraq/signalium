...
import {
  ...
  createWatcher as _createWatcher,
  SignalOptions,
  Signal,
  WriteableSignal,
  SignalSubscription,
...
export const state = <T>(initialValue: T, opts?: SignalOptions<T>): WriteableSignal<T> => {
  ...
  const s = _createState(initialValue, opts);
...
export const createComputed: typeof _createComputed = (compute, opts) => {
  const desc = opts?.desc || 'unlabeled';
  const counts = new SignalCounts(desc);

  const s = _createComputed(v => {
    counts.compute++;
...
export const createAsyncComputed: typeof _createAsyncComputed = (compute, opts) => {
  const desc = opts?.desc || 'unlabeled';
  const counts = new SignalCounts(desc);

  const s = _createAsyncComputed(async v => {
    counts.compute++;
...
export const createSubscription: typeof _createSubscription = (subscribe, opts) => {
  const desc = opts?.desc || 'unlabeled';
  const counts = new SignalCounts(desc);

  const s = _createSubscription((get, set) => {
    counts.subscribe++;
...
export function createWatcher<T>(fn: () => T, opts?: SignalOptions<T>): Watcher<T> {
  const desc = opts?.desc || 'unlabeled';
  const counts = new SignalCounts(desc);

  const w = _createWatcher<T>(() => {
    counts.compute++;
    return fn();
  }, opts);
