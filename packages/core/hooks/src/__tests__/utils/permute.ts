...
import { createAsyncComputed, createComputed, createSubscription, wrapHook } from './instrumented';
...
const createMethods = [
  ...
  },
  {
    ...
    create: (fn: (...args: any[]) => any) => {
      const computed = createAsyncComputed(fn);

      return wrapHook(computed, (...args: any[]) => {
  ...
  },
  {
    ...
    create: function _createSubscription<T, Args extends unknown[]>(fn: (...args: Args) => T): (...args: Args) => T {
      return createSubscription(({ set }, ...args) => {
        set(fn(...args));
