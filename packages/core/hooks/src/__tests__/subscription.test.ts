...
import { createSubscription } from './utils/instrumented';
import { nextTick } from './utils/async';

describe('subscriptions', () => {
  test('Subscription can set initial value', () => {
    const sub = createSubscription(({ set }) => {
      set(1);
    });
  ...
  test('Subscription can access parameters', () => {
    const sub = createSubscription(({ set }, value: number) => {
      set(value);
    });
